...
const { TrafficInsuranceFormData } = require("./types");
 ...
 */
async function testSompoQuoteProcessor(rawResult: any) {
  if (!rawResult) {
  ...
  try {
    ...
    console.log("\n💸 Taksit Seçenekleri:");
    processedResult.pricing.installmentOptions.forEach(
    ...
    if (processedResult.riskAnalysis.recommendations.length > 0) {
      ...
      processedResult.riskAnalysis.recommendations.forEach((rec: string) => {
        console.log(`     - ${rec}`);
      });
    }
